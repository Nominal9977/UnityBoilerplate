...
public class FlowFieldGUI : MonoBehaviour
{
    ...
    void AddInfluencePoint()
    {
        try
        {
            ...
            influenceYInput.text = "0";
            influenceStrengthInput.text = "1";
            UpdateStatusText($"Added repulsion point at ({x}, {y}) with strength {strength}");
        }
        catch (System.Exception e)
        {
    ...
    void CreateInfluenceListItem(FlowFieldGenerator.InfluencePoint point)
    {
    ...
    void RemoveInfluencePoint(FlowFieldGenerator.InfluencePoint point)
    {
        if (mapGeneration != null)
        {
    ...
    void ClearAllInfluencePoints()
    {
        if (mapGeneration != null)
        {
            ...
            var pointsToRemove = new List<FlowFieldGenerator.InfluencePoint>();
            ...
            UpdateInfluencePointList();
            UpdateStatusText($"Cleared {pointsToRemove.Count} repulsion points (kept start/target points)");
        }
    }
